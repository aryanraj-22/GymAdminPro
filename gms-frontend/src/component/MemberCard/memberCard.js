...
const MemberCard = ({item}) => {
    const [unknownImg, setUnknownImg] = useState("https://th.bing.com/th/id/OIP.gj6t3grz5no6UZ03uIluiwHaHa?rs=1&pid=ImgDetMain");
    return (
        <Link to={`/member/${item?._id}`} className='bg-white rounded-lg p-3 hover:bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:text-white cursor-pointer'>
            <div className='w-28 h-28 flex justify-center relative items-center border-2 p-1 mx-auto  rounded-full'>
                <img src={item?.profilePic} className='w-full h-full rounded-full' /> 
                <CircleIcon className='absolute top-0 left-0' sx={{color:item?.status==="Pending"?"red":"greenyellow",marginLeft:"5px"}}/>
            </div>
            ...
            <div className='mx-auto mt-1 text-center text-lg  font-mono'>
